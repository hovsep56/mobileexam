...
function rainbow(arrayLength: number, type: 'hex' | 'hsl' | 'rgb' = "hex", pastel: boolean = false): Color[] {
	...
	for (var i = 0; i < arrayLength; ++i) {
		...
		switch (type) {
			case 'hex':
				cvparr.push({ hex: rgbToHex(Math.round(red), Math.round(green), Math.round(blue)) } as HexColor);
				break;
			case 'rgb':
				cvparr.push({ r: Math.floor(red), g: Math.floor(green), b: Math.floor(blue) } as RgbColor);
				break;
			case 'hsl':
				cvparr.push(rgbToHsl(Math.round(red), Math.round(green), Math.round(blue)) as HslColor);
				break;
		}
	}
	return cvparr;
...
export { rainbow, Color, HexColor, RgbColor, HslColor };
